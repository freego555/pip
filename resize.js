...
module.exports = async (imgName, callback) => {
  try {
    ...
    const destFolderPath = config.has('destFolderPath') ? config.get('destFolderPath') : undefined
    const newWidth = config.has('newWidth') ? config.get('newWidth') : undefined
    const newHeight = config.has('newHeight') ? config.get('newHeight') : jimp.AUTO
    const watermarkImgPath = config.has('watermarkImgPath') ? config.get('watermarkImgPath') : undefined
    const opacitySource = config.has('opacitySource') ? config.get('opacitySource') : undefined
